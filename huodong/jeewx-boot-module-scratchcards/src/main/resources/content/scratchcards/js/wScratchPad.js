		...
		
{
	...
	ScratchPad.prototype = 
	{
		...
		{
			if($("#count").val() != '0' && $("#awardsNum").val() == 0){
				alert("总抽奖次数已用完！");
				return;
			}else if($("#remainNumDay").val() == 0){
				if($("#shareStatus").val() == '1' && $("#ifshare").val() == '0' && ($("#count").val() == '0' || $("#awardsNum").val() >= $("#remainNumDay").val() + 1)){
					alert("今日抽奖机会已用完!分享即可获得额外1次抽奖机会！");
				}else{
					alert("今日抽奖次数已用完！");
				}
				return;
			}else if($("#prizeStatus").val() == '1'){
				alert("您已经中过奖了，把机会留给其他人吧！");
				return;
			}else if($("#subscribeFlage").val() == '1'){
				return;
			}else{
